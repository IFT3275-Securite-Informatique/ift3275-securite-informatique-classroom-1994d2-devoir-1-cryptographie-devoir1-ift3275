...
def decrypt(C):
  ...
  rnd.seed(1337)  # Fixer la graine pour des résultats reproductibles
  int_keys = rnd.sample(range(256), len(symboles))
  K = {symbol: "{:08b}".format(k) for symbol, k in zip(symboles, int_keys)}

  # Créer la clé inversée
  reverse_key = {v: k for k, v in K.items()}
  ...
  while i < len(C):
      bits = C[i:i + 8]
      if bits in reverse_key:
          M += reverse_key[bits]
      else:
          M += "?"  # Caractère de remplacement pour les symboles inconnus
      i += 8
